...
import time
import numpy as np
#Six provides a consistent interface
...
def _register_custom_gradients():
    ...
    if not is_Registered:
        ...
        def _GuidedReluGrad(op, grad):
            return tf.where(0. < grad, nn.leaky_relu(grad, op.outputs[0]), tf.zeros_like(grad))
        is_Registered = True
    ...
    if isinstance(graph_or_sess, tf.Graph):
        ...
        for op in ops:
            if 'variable' in op.type.lower():
                raise ValueError('Please input a frozen graph , Or pass in the session object.')
...
def _get_visualization(sess_graph_path, value_feed_dict, input_tensor, layers, path_outdir, method = None):
    is_success = True
        ...
        with tf.Session(graph=g).as_default() as s:
            ...
            for layer in layers:
                if layer != None and layer.lower() not in dict_layer.keys():
                    is_success = _visualization_by_layer_name(g, value_feed_dict, input_tensor, layer, method, path_outdir)
                elif layer != None and layer.lower() in dict_layer.keys():
                    layer_type = dict_layer[layer.lower()]
                    is_success = _visualization_by_layer_type(g, value_feed_dict, input_tensor, layer_type, method, path_outdir)
                else:
...
def _graph_import_function(PATH, sess):
    ...
    return sess
def _visualization_by_layer_type(graph, value_feed_dict, input_tensor, layer_type, method, path_outdir):
    is_success = True
    ...
    for layer in layers:
        is_success = _visualization_by_layer_name(graph, value_feed_dict, input_tensor, layer, method, path_outdir)
    return is_success
def _visualization_by_layer_name(graph, value_feed_dict, input_tensor, layer_name, method, path_outdir):
    start = -time.time()
    is_success = True
    ...
    if deconv:
        is_success = write_results(results, layer_name, path_outdir, method = method)
...
def _deconvolution(graph, sess, op_tensor, X, feed_dict):
    ...
    return out
def deconv_visualization(sess_graph_path, value_feed_dict, input_tensor = None,  layers = 'r', path_outdir = "./Output"):
